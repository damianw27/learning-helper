...
export class RegisterStore extends ComponentStore<RegisterState> {
  ...
  readonly selectValidationErrors = this.select(
  ...
  readonly registerUser = this.effect(
    (registerFormData$: Observable<Record<string, string>>) => {
  ...
  private readonly requestUserRegistration = (
    registrationFormData: Record<string, string>
  ) =>
