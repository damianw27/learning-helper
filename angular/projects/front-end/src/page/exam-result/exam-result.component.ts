...
export class ExamResultComponent implements OnInit {
  ...
  loadExamInstance(params: Params): void {
    ...
    firstValueFrom(request$)
      .then(
        (receivedExamInstance) => (this.examInstance = receivedExamInstance)
      )
      .then((receivedExamInstance) =>
        this.loadExamDefinition(receivedExamInstance)
      )
      .finally(() => this.changeDetectorRef.detectChanges());
  }
  ...
  loadExamDefinition(receivedExamInstance: ExamInstance): void {
    const request$ = this.httpClient.get<ExamDefinition>(
      `/api/exam-definition/${receivedExamInstance.examDefinitionId}`
    );

    firstValueFrom(request$)
      .then(
        (receivedExamInstance) => (this.examDefinition = receivedExamInstance)
      )
      .finally(() => this.changeDetectorRef.detectChanges());
  }
}
