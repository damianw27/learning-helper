...
export class ExamComponent implements OnInit {
  ...
  loadExamInstance(params: Params): void {
    ...
    const request$ = this.httpClient.get<ExamInstance>(
      `/api/exam-instance/${examInstanceId}`
    );

    firstValueFrom(request$).then((receivedExamInstance) =>
      this.onGetExamInstanceSuccess(receivedExamInstance)
    );
  }

  onGetExamInstanceSuccess(receivedExamInstance: ExamInstance): void {
    this.examInstance = receivedExamInstance;
    let formGroupFields: Record<
      string,
      [any, ((control: AbstractControl) => ValidationErrors | null)[]]
    > = {};
    this.examInstance.questions.map(
      (question, index) =>
        (formGroupFields[`answer${index}`] = ['', [Validators.required]])
    );
  ...
  public onSubmit(): void {
    const answers = Object.values(this.answersForm.value).reduce(
    ...
    firstValueFrom(request$)
      .then(() =>
        this.router.navigateByUrl(`/exam-result/${this.examInstance.id}`)
      )
      .catch(console.error);
  }
