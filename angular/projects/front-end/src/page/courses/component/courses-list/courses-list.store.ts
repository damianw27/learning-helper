...
export class CoursesListStore extends ComponentStore<CoursesListState> {
  ...
  readonly setCourses = this.updater((state, courses: Course[]) => ({
    ...state,
    courses,
  }));

  readonly setPagesCount = this.updater((state, pagesCount: number) => ({
    ...state,
    pagesCount,
  }));
  ...
  readonly selectCourses = this.select((state) =>
    state.courses.slice(state.currentPage * 20, state.currentPage * 20 + 20)
  );
