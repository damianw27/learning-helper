...
export class LoginStore extends ComponentStore<LoginState> {
  ...
  readonly setCredentialsAreInvalid = this.updater((state, invalidCredentials: boolean) => ({ ...state, invalidCredentials }));

  readonly selectCredentialsAreInvalid = this.select(state => state.invalidCredentials);
}
