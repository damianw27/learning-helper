...
export class LessonComponent implements OnInit {
  ...
  private loadLesson(params: Params) {
    ...
    firstValueFrom(request$).then(responseCourse => this.onLessonLoadSuccess(responseCourse));
  }
  ...
  private onLessonLoadSuccess(responseLesson: Lesson) {
    ...
    responseLesson.lessonContents
      ...
      .forEach(lessonContentGroup => this.lessonContents.push(lessonContentGroup));

    this.lesson = responseLesson;

    const request$ = this.httpClient.get<Course>(`/api/course/${responseLesson.section.courseId}`);

    firstValueFrom(request$)
      .then(receivedCourse => (this.course = receivedCourse))
      .then(receivedCourse => (this.isEditMode = receivedCourse.author.id === this.loggedInUser.id))
  ...
  onSubmit($event: SubmitEvent) {
    const request$ = this.httpClient.post(`/api/lesson/update`, this.lessonForm.value);
  ...
  get contents(): LessonContent[] {
    return this.lesson.lessonContents.filter(lessonContent => lessonContent.learningStyle === this.loggedInUser.learningStyle);
  }
}
