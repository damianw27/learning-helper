...
export class CoursesElementComponent implements OnInit {
  ...
  constructor(private authService: AuthService, private httpClient: HttpClient, private changeDetectorRef: ChangeDetectorRef) {}

  public ngOnInit(): void {
    const request$ = this.httpClient.get<Participation>(`/api/participation/logged-user/${this.course.id}`);
  ...
  public sendParticipationRequest() {
    const request$ = this.httpClient.post(`/api/participation/${this.course.id}`, {});
    firstValueFrom(request$)
      .then(response => {
        if (response === null) {
          return;
