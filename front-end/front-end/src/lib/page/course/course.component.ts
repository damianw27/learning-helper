...
export class CourseComponent implements OnInit {
  ...
  public saveCourseChanges(): void {
    const request$ = this.coursesApiService.updateCourse(this.courseEditForm.value);
  ...
  public openExamsModal(): void {
    const modalRef = this.modalService.open(IncomingExamsComponent, { size: 'xl' });
  ...
  public openCreateQuestionModal(): void {
    ...
    modalRef.componentInstance.courseId = this.course.id;
    modalRef.componentInstance.questionCreated.subscribe((createdQuestion: Question) => {
  ...
  private loadCourse(params: Params) {
    ...
    firstValueFrom(this.courseApi.getCourse(courseId)).then(responseCourse => this.onCourseLoadSuccess(responseCourse));
  }
  ...
  private onCourseLoadSuccess(responseCourse: Course) {
    ...
    if (responseCourse.author.id === this.authService.getLoggedUser().id) {
      const request$ = this.httpClient.get<Question[]>(`/api/question/${responseCourse.id}`);
