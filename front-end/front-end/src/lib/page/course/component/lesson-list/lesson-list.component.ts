...
export class LessonListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const request$ = this.httpClient.get<Lesson[]>(`/api/lesson/by-section/${this.section.id}`);
  ...
  private onGetLessonsSuccess(receivedLessons: Lesson[]): void {
    this.lessons = receivedLessons.sort((a, b) => (a.cardinalNumber ?? 0) - (b.cardinalNumber ?? 0));
  }
