...
export class SectionElementComponent implements OnChanges {
  ...
  constructor(private httpClient: HttpClient, private formBuilder: FormBuilder, private changeDetectorRef: ChangeDetectorRef) {}
  ...
  public onFormSubmit(): void {
    const request$ = this.httpClient.post<Section>(`/api/section/update`, this.sectionForm.value);

    firstValueFrom(request$)
      .then(updatedSection => this.updateSectionAndRefreshView(updatedSection))
      .catch(console.error);
  }
