...
export class AuthService {
  ...
  public login(authorizationToken: string): Observable<UserAuth> {
    ...
    return this.httpClient.get<UserAuth>('/api/user/login', { headers }).pipe(tap(response => (this.loggedUser = response.user)));
  }
  ...
  public getLoggedUser(): User {
    if (this.loggedUser !== undefined) {
      ...
      if (this.loggedUser.learningStyle === null && !this.isQuestionnaireShown) {
